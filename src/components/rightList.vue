...
<template>
  <div class="rightlistBox">
    <section>
      <div class="r1-head">
        <img
          :src="userDataStore.themeObj.center_smailimg
               ?userDataStore.themeObj.center_smailimg
               :userBg
          "
          alt=""
        />
        <h1 v-if="userDataStore.themeObj.user_start !== 0">
          <span>让真实的声音回响！</span>
        </h1>
      </div>
      <div class="r1-body">
    ...
    <!-- 右侧上滑小图片 -->
    <div
      v-if="userDataStore.themeObj.user_start != 0"
      :class="gotoTop ? 'toTop hidden' : 'toTop goTop hidden'"
      @click="toTopfun()"
    >
      <img
        :src="userDataStore.themeObj.right_img
             ?userDataStore.themeObj.right_img
             :scroll
        "
        alt=""
      />
...
<script setup lang="ts" name="rightList">
...
const fixDo =  ref(false)
const gotoTop = ref(false)//返回顶部
const going =  ref(false) //是否正在执行上滑动作
const browseList = ref() //热门文章 浏览量最多
const artCommentList = ref("")//最新评论
const catchMeObj = {
  //个人信息
  qq: "https://im.qq.com",
...
onMounted(async ()=>{
  browseList.value = await hotArticleList()
  }
)
</script>
