            ...
            <header>
                ...
                <div class="ui label">
                    <a @click="router.push({path:'/Share',query:{
                        classId:detailObj.categoryId}})">{{detailObj.categoryName}}</a>
                </div>
            </header>
...
const aid =ref()//文章ID
const detailObj = ref({})//返回详情数据
const userId=ref('')//用户id
const userInfo=ref('')

function showInitDate(date,full){//年月日的编辑
    return initDate(date,full);
}
async function getArticleDetail(){
    const response = await getArticle(aid.value)
    detailObj.value = response
    const md = markdownit()
    detailObj.value.content = md.render(response.content);
}
