                    ...
                    <div class="OwO-body">
                        <ul class="OwO-items OwO-items-show">
                            <li class="OwO-item" v-for="(oitem,index) in OwOlist" :key="'oitem'+index" @click="choseEmoji(oitem.title)">
                                <img :src="'../../public/emot/'+oitem.url" alt="">
                            </li>
                        </ul>
                        <div class="OwO-bar">
            ...
            <div class="tmsg-commentshow">
                <ul class="tmsg-commentlist">
                    <li class="tmsg-c-item" v-for="(item,index) in commentList" :key="'common'+index">
                        <article class="">
                            <header>
                                <img  :src="userDataStore.errorImg"  :onerror="userDataStore.errorImg">
                                <div class="i-name">
                                    {{item.username}}
                                </div>
                                <div class="i-time">
                            ...
                            <section>
                                ...
                                <div v-if="userDataStore.haslogin" class="tmsg-replay" @click="respondMsg($event,item.id,item.id,item.createBy)">
                                    回复
                                </div>
                            </section>
                        </article>
                            ...
                            <li class="tmsg-c-item" v-for="(citem,cindex) in item.children" :key="'citem'+cindex">
                                <article class="">
                                    ...
                                    <section>
                                        ...
                                        <div v-show="userDataStore.haslogin" class="tmsg-replay" @click="respondMsg($event,item.id,citem.id,citem.createBy)">
                                            回复
                                        </div>
                                    </section>
                                </article>
...
const respondBox = ref('')//评论表单
const listDom = ref('')//评论列表
const tmsgBox = ref()//总评论盒子
// 查询参数
...
const textarea= ref('')//文本框输入内容
const pBody= ref(true)//表情打开控制
const commentList= ref([])//评论列表数据
const aid= ref(0)//文章id
const hasMore= ref(false)
const userId= ref('')//用户id
const type= ref(0)//回复评论的当前的commentId
const leavePid= ref('')//赞赏等其他模块的分类id
const pid= ref('')//回复评论的一级commentId
...
function setData(initData,result){
    ...
    if(initData){
        //刷新列表
        commentList.value = msg;
    }else{
        //加载更多
        commentList.value = commentList.value.concat(msg);
    }
    hasMore.value = result.total > commentList.value.length
}
//选择表情包
...
function analyzeEmoji(cont){//编译表情替换成图片展示出来
    ...
    const pattern2 = /\[[\u4e00-\u9fa5]+\]/;
    const content = cont.match(pattern1);
    let str = cont;
    let src = ''
    if(content){
        for(let i=0;i<content.length;i++){
            for(let j=0;j<OwOlist.values.length;j++){
                if("["+OwOlist.values[j].title +"]" == content[i]){
                    src = OwOlist.values[j].url;
                    break;
                }
            }
            str = str.replace(pattern2,'<img src="../../public/emot/'+src+'"/>');
        }
    }
    return str;
}
//发送留言
async function sendMsg(){//留言
    if(textarea.value){
        sendTip.value = '咻~~';
        await sendComment(type.value,aid.value,rootId.value,toCommentId.value,toCommentUserId.value,textarea.value)
        textarea.value = '';
        rootId.value = -1;
        toCommentId.value = -1;
...
function respondMsg($event,rootId1,toCommentId1,toCommentUserId1){//回复留言
...
function showCommentList(initData){//评论列表
    aid.value =  route.query.aid==undefined?1:parseInt(route.query.aid as string);//获取传参的aid
    queryParams.value.articleId = aid.value
    //判断当前用户是否登录
    const token = userDataStore.token;
    if(token){
    ...
    if(route.name=='DetailArticle'){//文章列表的评论
        type.value = 0;
        getArticleComment(queryParams.value).then((response)=>{
            setData(initData,response);
        })
    }else{//其他评论
        if(route.name == 'FriendsLink'){
            type.value = 1
            getLinkComment(queryParams.value).then((response)=>{
                setData(initData,response);
            })
        }
    }
