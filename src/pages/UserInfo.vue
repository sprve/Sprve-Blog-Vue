                    ...
                    <ul class="userInfoBox">
                        <li class="avatarlist">
                            ...
                            <div class="avatar-uploader">
                                <img  :src="userInfoObj.avatar?userInfoObj.avatar:userDataStore.errorImg"   :onerror="userDataStore.errorImg" class="avatar">
                            </div>
                        </li>
                        <li class="username">
                            <span class="leftTitle">昵称</span>
...
let userInfo={}                       //本地存储的用户信息
const uploadURL=ref('')               //上传路径
const isEdit= ref(false)              //是否编辑状态
const userInfoObj=ref('')             //用户的信息/事件处理器
            
function handleAvatarSuccess(res, file) {//上传头像
    if(res.code == 200){
        userInfoObj.value.avatar= res.data
        userInfoObj.value.head_start = 1;
    }else{
        ElMessage.error('上传图片失败');
    }
}
function beforeAvatarUpload(file) {//判断头像大小
...
async function saveInfoFun(){//保存编辑的用户信息
    if(!userInfoObj.value.nickName){ //昵称为必填
        ElMessage.error('昵称为必填项，请填写昵称');
        return;
    }
    await saveUserInfo(userInfoObj.value)//保存信息接口，返回展示页
    ElMessage.success( '保存成功！');
    isEdit.value = false;
    routeChange();
}
async function routeChange(){//展示页面信息
    if(localStorage.getItem('userInfo')){
        console.log(userDataStore.haslogin)
        userDataStore.setUserHasLogin(true);
        userInfo = JSON.parse(localStorage.getItem('userInfo'));
        getUserInfo(userInfo.id).then((response)=>{
            userInfoObj.value = response;
            userInfoObj.value.head_start = 0;
        })
    }else{
